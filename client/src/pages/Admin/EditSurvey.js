...
const EditSurvey = () => {
  ...
  const fetchSurvey = async () => {
    try {
      const response = await axios.get(`/admin/surveys/${id}`);
      const surveyData = response.data;
      setSurvey(surveyData);
      
      // Reset form with survey data
      reset({
        title: surveyData.title,
        description: surveyData.description,
        rewardAmount: surveyData.rewardAmount,
        estimatedTime: surveyData.estimatedTime,
        maxCompletions: surveyData.maxCompletions || '',
  ...
  const onSubmit = async (data) => {
    ...
    try {
      setSubmitting(true);
      await axios.put(`/admin/surveys/${id}`, surveyData);
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-600 mt-2">Update survey details and questions.</p>
        {survey && (
          <div className="mt-4 text-sm text-gray-500">
            Created by {survey.createdBy} on {new Date(survey.createdAt).toLocaleDateString()}
            {survey.currentCompletions > 0 && (
              <span className="ml-4 text-orange-600">
                ⚠️ This survey has {survey.currentCompletions} completions. Changes may affect data consistency.
              </span>
            )}
          </div>
        )}
            ...
            <div className="space-y-6">
              <div className="flex items-center mb-4">
                ...
                <label className="ml-2 text-sm font-medium text-gray-700">
                  Survey is active and available to users
                </label>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div>
                  ...
                  {survey && survey.currentCompletions > 0 && (
                    <p className="text-xs text-gray-500 mt-1">
                      Current completions: {survey.currentCompletions}
                    </p>
                  )}
                </div>
              </div>
